...
export type LemmaOpenCodeCredentials = {
  version: 1;
  apiUrl: string;
  projectId: string;
  credentialId: string;
...
export type LemmaOpenCodeCredentialScope = Pick<
  LemmaOpenCodeCredentials,
  "apiUrl" | "projectId" | "credentialId"
>;
...
export const LEMMA_OPENCODE_CREDENTIALS_HELP =
  "Lemma OpenCode credentials are missing or invalid. Run `pnpm dlx @uselemma/opencode setup` to connect or rotate the scoped credential.";

function isRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}

function isCredentials(value: unknown): value is LemmaOpenCodeCredentials {
  return (
    isRecord(value) &&
    value.version === 1 &&
...
export function resolveConfigDir(options: StorageOptions = {}): string {
  ...
  return xdgConfigHome
    ? join(resolve(xdgConfigHome), "opencode")
    : join(resolve(home), ".config", "opencode");
}
...
export function pluginPath(options: StorageOptions = {}): string {
...
): Promise<LemmaOpenCodeCredentials | null> {
  try {
    const value = JSON.parse(
...
export async function writeCredentials(
  credentials: LemmaOpenCodeCredentials,
  options: StorageOptions = {},
): Promise<void> {
...
export function configDirExists(options: StorageOptions = {}): boolean {
  return existsSync(resolveConfigDir(options));
}
