...
Setup copies the bundled plugin into `~/.config/opencode/plugins/uselemma-opencode.js`, opens project-scoped Lemma authorization, and stores the resulting credential under `~/.config/opencode/lemma`. OpenCode automatically loads global plugin files at startup.
...
OpenCode supports both local plugin files and npm package configuration. The setup command uses the local-file path so it can be verified from a checkout or packed tarball before any public npm publication. Public npm release and ecosystem listing are separate release steps.
