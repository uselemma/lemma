...
`runtime/` and `scripts/` are generated by `pnpm --filter @uselemma/opencode build`. They are not committed. CI, `verify:local`, and publish build them before test, pack, and npm publish.
