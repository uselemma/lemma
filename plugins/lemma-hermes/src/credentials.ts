...
export type LemmaHermesCredentials = {
  version: 1;
  apiUrl: string;
  projectId: string;
  credentialId: string;
...
type DataDirLocation = {
  version: 1;
  dataDir: string;
};

export const LEMMA_HERMES_CREDENTIALS_HELP =
  "Lemma Hermes credentials are missing or invalid. Run `pnpm dlx @uselemma/hermes setup` to connect or rotate the scoped credential.";

function isRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}

function isCredentials(value: unknown): value is LemmaHermesCredentials {
  return (
    isRecord(value) &&
    value.version === 1 &&
...
export function resolveHermesHome(options: StorageOptions = {}): string {
...
): Promise<LemmaHermesCredentials | null> {
  try {
    const value = JSON.parse(
      await readFile(credentialsPath(options), "utf8"),
    ) as unknown;
    if (!isCredentials(value)) throw new Error(LEMMA_HERMES_CREDENTIALS_HELP);
    return value;
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === "ENOENT") return null;
    if (error instanceof SyntaxError)
      throw new Error(LEMMA_HERMES_CREDENTIALS_HELP);
    throw error;
  }
}

export async function writeCredentials(
  credentials: LemmaHermesCredentials,
  options: StorageOptions = {},
): Promise<void> {
