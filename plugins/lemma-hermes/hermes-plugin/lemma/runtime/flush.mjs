...
// src/flush.ts
import { mkdir as mkdir2, readFile as readFile2, readdir, rm, stat, unlink as unlink2 } from "node:fs/promises";
...
function createDeliveryFetch(fetchImplementation = fetch, timeoutMilliseconds = DELIVERY_TIMEOUT_MS) {
  return async (request, init) => {
    ...
    const timedOut = new Promise((_, reject) => {
      timeout = setTimeout(() => {
        controller.abort();
        reject(new Error("Lemma Hermes trace delivery timed out"));
      }, timeoutMilliseconds);
    });
    try {
...
function isPendingTurn(value) {
  return isRecord2(value) && value.version === 1 && typeof value.apiUrl === "string" && typeof value.projectId === "string" && isHermesTurn(value.turn);
}
function mapHermesTurn(turn) {
...
async function flushPendingTurns(options = {}) {
  ...
  try {
    ...
    const entries = await readdir(pendingDir).catch((error) => {
