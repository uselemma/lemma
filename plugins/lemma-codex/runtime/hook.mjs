...
async function readPendingTurn(path) {
  const value = await readJson(path);
  if (value === null) return null;
  if (!isPendingCodingAgentTurn(value)) {
...
async function reconcileTranscriptToolFailures(turn, input, fallbackEndedAt, warn) {
  ...
  let transcript;
  try {
    transcript = await readFile2(path, "utf8");
  } catch (error) {
  ...
  for (const line of transcript.split("\n")) {
    ...
    try {
      ...
      if (!isRecord2(payload) || payload.type !== "function_call_output" || typeof payload.call_id !== "string") {
        continue;
      }
...
async function defaultSendTrace(input) {
  ...
  const fetchWithTimeout = async (request, init) => fetch(request, { ...init, signal: controller.signal });
...
async function flushPendingTurns(dependencies = {}) {
  ...
  if (dependencies.waitUntilReady) {
    ...
    const delays = pending.map(
      ({ readyAt }) => readyAt ? Math.max(0, Date.parse(readyAt) - Date.now()) : 0
    );
    const delay = delays.length > 0 ? Math.min(...delays) : 0;
    if (delay > 0) {
      await new Promise(
        (resolvePromise) => setTimeout(resolvePromise, delay)
      );
    }
  }
    ...
    for (const listed of await listPendingTurns(dataDir)) {
      ...
      const deliverable = await withSessionLock(
        ...
        async () => {
          ...
          if (latest.sourceRevision) {
            ...
            if (open && codingAgentTurnRevision(open) !== latest.sourceRevision) {
      ...
      try {
        ...
        await withSessionLock(dataDir, pending.turn.sessionId, async () => {
          ...
          const sameDelivery = pending.deliveryId ? latest?.deliveryId === pending.deliveryId : latest?.turn.endedAt === pending.turn.endedAt;
          ...
          if (!pending.sourceRevision || open && codingAgentTurnRevision(open) === pending.sourceRevision) {
            await removeTurn(
              dataDir,
              pending.turn.sessionId,
              pending.turn.turnId
...
${prompt}`,
  ...
  const settleDelayMs = dependencies.settleDelayMs ?? (dependencies.sendTrace ? 0 : STOP_SETTLE_DELAY_MS);
