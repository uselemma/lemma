...
meta:
  ...
  goal: "Read project-level volume, latency, errors, tools, models, and cost"
  audience: "People watching trends across many traces"
---
...
Analytics is feature-gated. If you don’t see **Analytics** in the project sidebar, it isn’t enabled for your workspace. The project **Dashboard** (home and setup) is a different page. See [Product boundaries](/platform/concepts#product-boundaries).
...
Analytics is project-scoped, and the agent scorecard lists every agent that appeared in the window. Agent names come from the traces themselves, not from a registry you maintain elsewhere. A missing agent means it didn’t emit ready traces in this project and window, not that someone deleted it from a catalog.
