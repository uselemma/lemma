...
An instrumented agent emits one trace per execution, or per conversation turn when you set a thread ID. Lemma stores the trace's spans, marks it ready, analyzes it for behavior that hurt the agent's task, and groups repeated evidence of the same failure into one issue. See [Concepts](/platform/concepts) for how detection and grouping work, and why both are probabilistic.
