...
<Note>
  **One agent execution = one trace.** Child LLM calls, tool calls, retrieval, and app logic belong inside that root — not as separate top-level traces. `threadId` groups turns of a conversation; it does not merge a host and a sandbox that are one turn.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="One turn across processes" href="/tracing/instrumentation/cross-process-turns">
    Host + sandbox into one root, not two traces that share a thread id.
  </Card>
</CardGroup>
