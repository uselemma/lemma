...
Turns often do not run in the same process: a request handler answers turn 1, a queue worker answers turn 2, or turns are processed asynchronously. The rule is unchanged: start a new top-level trace per turn. Carry the thread id and user id across the boundary so the new process can reattach the turn to the conversation.
...
Export a context token on the host. Record a journal in the child with `attachTurn` / `attach_turn` and no Lemma API key. Apply the journal on the host, then `ingest()` once. See [Cross-process turns](/tracing/instrumentation/cross-process-turns).
