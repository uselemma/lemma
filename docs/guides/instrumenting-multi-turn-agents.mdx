...
Two turns, two traces, one thread. Everything a single turn does still nests inside that turn’s trace: LLM calls, tools, retrieval, and app logic, exactly as it would for a one-shot agent.

## Root input versus generation messages

The split that trips people up is between the root `input` and the generation’s message history.

- **Root `input`**: the current user turn only, the single message that started this execution.
- **The generation**: the full ordered message list sent to that model call, including the system prompt, prior conversation turns, tool results, and the current user message. That list is what the model saw.
...
Turns may run in different processes: a request handler answers turn 1, a queue worker answers turn 2, or turns run asynchronously. The rule is unchanged: start a new top-level trace per turn, but pass the thread id (and user id) across the boundary so the new process can reattach the turn to the conversation.
...
<Tabs>
  <Tab title="TypeScript">
    ```typescript
    // Producer: pass the thread id to the job that will answer the turn.
...
Both turns share `threadId` / `thread_id` and `userId` / `user_id`. Turn 1 is a plain answer. Turn 2 calls a tool and sends the growing message history to the model: system prompt, the turn 1 exchange, the tool result, and the current user message.
...
## Checklist for multi-turn traces
...
## Patterns that break conversation grouping
...
<Warning>
  ...
  - **Putting the full history only on the root**: the root `input` is the current turn. Prior turns and tool results belong on the generation’s message list, otherwise you can’t see what the model received.
  ...
  - **Incremental-ingesting one turn**: send each turn as one complete trace when it finishes; don’t stream sparse partial deliveries under the same `trace.id` expecting later calls to patch the root or re-run analysis. Retries of the same complete payload are safe.
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Building high-quality traces" icon="/icons/circle-check.svg" href="/guides/building-high-quality-traces">
    Bad, better, and best examples for the ideal instrumentation path.
  </Card>
