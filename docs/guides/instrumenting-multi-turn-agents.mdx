...
meta:
  contentType: "How-to"
  goal: "Link successive conversation turns with one thread id while keeping each turn as its own trace"
  audience: "Engineers with multi-turn agents"
---
...
<Note>
  **One agent execution = one trace still holds.** A conversation turn is one execution, so it is one trace. The `threadId` groups the turns; it does not merge them into a single trace.
</Note>
...
Turns do not always run in the same process. A request handler can answer turn 1 while a queue worker answers turn 2, or turns can run asynchronously. The rule is unchanged: start a new top-level trace per turn. Carry the thread id and user id across the boundary so the new process can reattach the turn to the conversation.
...
<Note>
  This section covers successive user turns. If one turn spans a host and a sandbox, keep a single root. See [Cross-process turns](/tracing/instrumentation/cross-process-turns).
</Note>
...
## Record one turn on a host and sandbox

A host plus a sandbox such as E2B is still one user turn, so it is one trace:
...
See [Cross-process turns](/tracing/instrumentation/cross-process-turns) for `startTurn` / `attachTurn` / `apply`.
...
A well-instrumented multi-turn agent records:
...
- A host and sandbox that are one user turn share one root via `startTurn` / `attachTurn` / `apply`.
...
<Warning>
  ...
  - **Host and sandbox as two traces.** If one user turn spans processes, use `startTurn` / `attachTurn` / `apply`. See [Cross-process turns](/tracing/instrumentation/cross-process-turns).
...
<CardGroup cols={2}>
  ...
  <Card title="Cross-process turns" href="/tracing/instrumentation/cross-process-turns">
    `startTurn` / `attachTurn` / `apply` when one turn runs on a host and a sandbox.
  </Card>
</CardGroup>
