...
<Note>
  This is for **successive user turns**. If **one** turn is split across a host and a sandbox, do not start a second root with the same `threadId` — see [Cross-process turns](/tracing/instrumentation/cross-process-turns).
</Note>
...
See [Cross-process turns](/tracing/instrumentation/cross-process-turns) for the API (`export`, `apply`, `assembleTurn` / `assemble_turn`) and a host + child example.
...
<Warning>
  ...
  - **Host + sandbox as two traces.** If one user turn spans processes, use `startTurn` / `attachTurn` / `apply` — see [Cross-process turns](/tracing/instrumentation/cross-process-turns).
