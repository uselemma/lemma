...
A host orchestrator plus an E2B-style sandbox is one user turn. The host mints a context token (`startTurn` / `start_turn`), the child records a journal with `attachTurn` / `attach_turn` and no Lemma API key, and the host `apply`s the journal then `ingest()`s once.
...
- A host + sandbox that is **one** user turn is one root via `startTurn` / `attachTurn` / `apply`.
...
<Warning>
  ...
  - **Host + sandbox as two traces.** If one user turn spans processes, use `startTurn` / `attachTurn` / `apply` — see [One turn across processes](/tracing/instrumentation/cross-process-turns).
...
<CardGroup cols={2}>
  ...
  <Card title="One turn across processes" href="/tracing/instrumentation/cross-process-turns">
    `startTurn` / `attachTurn` / `apply` when one turn spans a host and a sandbox.
  </Card>
</CardGroup>
