...
meta:
  ...
  goal: "Fix missing traces and trace-shape problems by symptom"
  audience: "Engineers debugging ingest or dashboard display"
---
...
- **One user turn** on a host and a sandbox: export a context token, record a journal in the child with no API key, then `apply` and `ingest()` once. See [Cross-process turns](/tracing/instrumentation/cross-process-turns).
...
**Fix:** export a context token on the host. Call `attachTurn` / `attach_turn` in the child with no API key. Apply the journal on the host and `ingest()` once. Keep `LEMMA_API_KEY` off the sandbox.
...
See [Cross-process turns](/tracing/instrumentation/cross-process-turns). Set `threadId` on the host root so this turn groups with the rest of the conversation.
...
**Cause:** the trace was delivered again with a sparse `ingest()` payload, for example a follow-up call that carries a late child span but omits root `input`/`output`. `ingest()` is not an incremental merge API, so omitted root fields do not preserve prior values, and analysis already ran once after the first complete delivery.

**Fix:** send **one complete trace** when the execution finishes: root input/output, thread/user, and all child spans in a single `ingest()` call. Retries of that same complete payload are safe (already-stored span IDs are skipped). Across queues and workers, assemble the full tree in a coordinator and ingest once rather than streaming partial deliveries under one `trace.id`. For a host and sandbox that are one turn, apply the child journal on the host and ingest once. See [Cross-process turns](/tracing/instrumentation/cross-process-turns).
...
## Traces render but there are no issues
...
- If one turn spans a host and a sandbox, use `startTurn` / `attachTurn` / `apply`. See [Cross-process turns](/tracing/instrumentation/cross-process-turns).
