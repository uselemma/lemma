...
This page is organized by **symptom**. Most issues are trace-shape problems: the data arrives, but it does not match the [trace contract](/reference/trace-contract). For the ideal happy path and bad → better → best examples, see [Building high-quality traces](/guides/building-high-quality-traces).
...
## Audit traces that already land

Use the Lemma diagnostics skill when traces are arriving and you want to check whether their shape matches the [high-quality traces guide](/guides/building-high-quality-traces). The skill audits your traces without editing your app.
...
```text
Are these traces high quality? Audit the shape of traces already in Lemma.
```

If no traces are arriving, start with [Quickstart](/tracing/instrumentation/setup) instead.
...
- **Successive conversation turns** in different processes (request handler answers turn 1, a worker answers turn 2): start a **new** root for each turn and pass the same `threadId` / `userId`. See [Instrumenting multi-turn agents](/guides/instrumenting-multi-turn-agents).
- **One user turn** split across a host and a sandbox (E2B-style): do **not** start a second root with the same `threadId`. The host mints a context token, the child records a journal with no API key, and the host `apply`s then `ingest()`s once. See [One turn across processes](/tracing/instrumentation/cross-process-turns).

**Debug with Debug mode:** compare logs from the request handler and any background worker. If the child work runs after the `lemma.trace()` callback has returned, it will not attach to the root trace automatically. Move the recording into the callback, pass trace/span IDs to detached `record*` helpers from the coordinator, or use `startTurn` / `attachTurn` when the child is a separate process of the **same** turn.

## Host and sandbox show up as two traces

**Cause:** each process started its own `lemma.trace()` and shared `threadId` / `thread_id`. Lemma groups those as two conversation turns, not one execution.

**Fix:** for one user turn split across a host and an E2B-style sandbox, mint a context token on the host, `attachTurn` / `attach_turn` in the child (no API key), `apply` the journal on the host, and `ingest()` once. Do not put `LEMMA_API_KEY` in the sandbox or stream partial traces to Lemma.
...
See [One turn across processes](/tracing/instrumentation/cross-process-turns). `threadId` still belongs on the host root so this turn groups with the rest of the conversation.

**Debug with Debug mode:** the host should log one `sending trace` / `trace sent` with sandbox children included. The child process should not log ingest at all.
...
**Cause:** the trace was delivered again with a sparse `ingest()` payload — for example a follow-up call that carries a late child span but omits root `input`/`output`. `ingest()` is not an incremental merge API, so omitted root fields do not preserve prior values, and analysis typically already ran once after the first complete delivery.

**Fix:** send **one complete trace** when the execution finishes — root input/output, thread/user, and all child spans in a single `ingest()` call. Retries of that same complete payload are safe (already-stored span IDs are skipped). Across queues and workers, assemble the full tree in a coordinator and ingest once rather than streaming partial deliveries under one `trace.id`. For a host + sandbox that is one turn, `apply` the child journal on the host and ingest once — see [One turn across processes](/tracing/instrumentation/cross-process-turns).

**Debug with Debug mode:** check the `sending trace` log for each delivery. If a later send has a small `spanCount` (`span_count` in Python) and no root I/O, that sparse payload is overwriting the root fields — consolidate it into the single terminal send.
...
**Debug with Debug mode:** check whether the ingest log appears before the function returns. If it appears after the response is sent, or never appears, await the trace promise and avoid detached background recording unless you explicitly end the trace handle.
...
- If one turn spans a host and a sandbox, stitch them with `startTurn` / `attachTurn` / `apply` rather than two roots that share `threadId`.
