...
- **One user turn** split across a host and a sandbox (E2B-style): the host mints a context token, the child records a journal with no API key, and the host `apply`s then `ingest()`s once. See [One turn across processes](/tracing/instrumentation/cross-process-turns).
...
- If one turn spans a host and a sandbox, use `startTurn` / `attachTurn` / `apply` — see [One turn across processes](/tracing/instrumentation/cross-process-turns).
