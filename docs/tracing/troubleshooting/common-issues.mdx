...
- **One user turn** split across a host and a sandbox (E2B-style): the host mints a context token, the child records a journal with no API key, and the host `apply`s then `ingest()`s once. See [Cross-process turns](/tracing/instrumentation/cross-process-turns).
...
See [Cross-process turns](/tracing/instrumentation/cross-process-turns). `threadId` still belongs on the host root so this turn groups with the rest of the conversation.
