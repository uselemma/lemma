...
meta:
  ...
  goal: "Diagnose whether the SDK created, sent, and ingested a trace"
  audience: "Engineers whose traces are missing, delayed, or malformed"
---
...
| `trace sent` status 200 | none | Wrong endpoint (`/otel/v1/traces` or mock) |
...
| `trace sent` status 201 + verify `not_found` | missing enqueue index / CH row | Downstream lag or failure |
...
| `trace started`, but no `sending trace` | The callback did not finish, or the trace handle was never ended | Await `lemma.trace(...)`, call `trace.end(...)`, and check thrown errors |
