...
If `baseUrl` is not production, `projectId` is not a valid UUID, or the ingest path differs, the banner includes `warnings`.
...
After a successful 201, optionally poll the API to confirm traces are visible:

```bash
export LEMMA_DEBUG_VERIFY="true"
```

With verify enabled, the SDK calls `GET /traces/has-ready?project_id=...` after ingest and logs one of:
