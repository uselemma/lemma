...
If `baseUrl` is not production or `projectId` is not a valid universally unique identifier, the banner includes `warnings`.
...
`enqueued` means the OpenTelemetry Protocol worker accepted the payload and wrote it to the ingest queue. It does not require the dashboard to show the trace yet (useful during backlog).
