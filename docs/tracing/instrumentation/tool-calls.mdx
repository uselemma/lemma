...
A _tool call_ is a single tool invocation inside a trace. Record it with `trace.recordTool()` / `trace.record_tool()` after the tool returns.

## Record a tool call

Record a tool call once the tool returns, passing the arguments, result, and duration:
...
If a tool call fails, record the failure before rethrowing:
