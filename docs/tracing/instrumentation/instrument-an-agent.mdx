...
One agent execution is one `lemma.trace()` call. Record model calls, tool calls, and app work inside that trace. If that execution spans a host and a sandbox, use [One turn across processes](/tracing/instrumentation/cross-process-turns) instead of a second root.
...
<CardGroup cols={2}>
  ...
  <Card title="Vercel AI SDK" href="/integrations/vercel-ai">
  ...
  <Card title="One turn across processes" href="/tracing/instrumentation/cross-process-turns">
    Host + sandbox journal instead of two roots.
  </Card>
</CardGroup>
