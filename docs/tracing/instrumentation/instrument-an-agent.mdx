...
One agent execution is one `lemma.trace()` call. Record model calls, tool calls, and app work inside that trace. If that execution spans a host and a sandbox, use [Cross-process turns](/tracing/instrumentation/cross-process-turns) instead of a second root.
