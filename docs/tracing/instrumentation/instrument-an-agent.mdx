...
meta:
  ...
  goal: "Add each SDK primitive to an untraced agent until the full shape is recorded"
  audience: "Engineers who finished the quickstart and need the full shape"
---

This page starts from an untraced support agent and adds Lemma software development kit (SDK) calls one piece at a time. Complete the [Quickstart](/tracing/instrumentation/setup) first so the SDK is installed and a Lemma client can read your credentials.

The per-primitive pages go deeper on each piece: [Agent traces](/tracing/instrumentation/traces), [Generations](/tracing/instrumentation/generations), [Tool calls](/tracing/instrumentation/tool-calls), [Spans](/tracing/instrumentation/spans), and [Threads and context](/tracing/instrumentation/context).

One agent execution is one `lemma.trace()` call. Record model calls, tool calls, and app work inside that trace. If that execution spans a host and a sandbox, follow [Cross-process turns](/tracing/instrumentation/cross-process-turns) instead of starting a second root.
...
<CardGroup cols={2}>
  ...
  <Card title="Vercel AI SDK" href="/integrations/vercel-ai" icon="/images/brands/invert/vercel-ai.svg">
    Record AI SDK model calls and tool executions automatically.
  </Card>
  <Card title="Troubleshooting" href="/tracing/troubleshooting/common-issues">
