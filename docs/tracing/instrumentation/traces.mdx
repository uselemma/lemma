...
When one user turn spans a host and a sandbox, use `startTurn` / `attachTurn` / `apply` (Python `start_turn` / `attach_turn`) so they become one root — see [One turn across processes](/tracing/instrumentation/cross-process-turns).
