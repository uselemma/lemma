...
This page is how you **create** traces in the SDK. To search, filter, and inspect ready traces in the dashboard, see [Traces](/platform/traces).
...
| --- | --- | --- |
| **Root / run latency** | Trace `started_at` → `ended_at` / `duration_ms` | End-to-end agent execution time |
| **Generation duration** | Generation span `duration_ms` (or its `started_at` → `ended_at`) | Time for a single model call |
...
Framework integrations already set both. For manual instrumentation, pass `durationMs` on each generation and let the root measure from `lemma.trace()` start to `end()` / callback return.
...
<Tabs>
  <Tab title="TypeScript">
    ...
    context.recordGeneration({ name: "answer", model: "gpt-4o", output: finalAnswer });
...
`threadId` / `thread_id` correlates **turns of a conversation**. It does not stitch a host process and an E2B-style sandbox into one turn.

When one user turn spans processes, the host calls `startTurn` / `start_turn`, the child `attachTurn`s / `attach_turn`s a context token and records a journal **without** a Lemma API key, and the host `apply`s that journal then `ingest()`s once. Do not start a second root with the same `threadId`. There is no live span stream to Lemma from the sandbox.

See [One turn across processes](/tracing/instrumentation/cross-process-turns) for the host + child example, `assembleTurn` / `assemble_turn`, and unclean-exit handling.
...
If the callback throws, the SDK records the trace as failed, sends it to Lemma, and rethrows the original error. Automatic delivery (`trace` / `async_trace` / `TraceHandle.end()`) fails open: a Lemma ingest 4xx/5xx or network error is logged in debug mode and dropped, so it cannot fail the caller's application or replace the original error. A transport failure on a successful run is not recorded as an agent failure. Use `ingest()` when you need a failed send to raise so you can retry.
...
<CardGroup cols={2}>
  ...
  <Card title="Traces in the dashboard" href="/platform/traces">
    Search and inspect the traces you send.
  </Card>
  ...
  <Card title="One turn across processes" href="/tracing/instrumentation/cross-process-turns">
    Host + sandbox into one ingest payload.
  </Card>
</CardGroup>
