...
meta:
  contentType: "How-to"
  goal: "Wrap one agent execution in one trace with input and output"
  audience: "Engineers using the Lemma SDK directly"
---
...
When one user turn spans a host and a sandbox, use `startTurn` / `attachTurn` / `apply` (Python `start_turn` / `attach_turn`) so they become one root. See [Cross-process turns](/tracing/instrumentation/cross-process-turns).
