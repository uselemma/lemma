...
```bash
export LEMMA_RELEASE="<stable-deployment-identifier>"
```
...
<Tabs>
  <Tab title="TypeScript">
    ...
    const lemma = new Lemma({
      release: "<stable-deployment-identifier>",
    });
    ```
  </Tab>
  <Tab title="Python">
    ```python
    lemma = Lemma(release="<stable-deployment-identifier>")
    ```
  </Tab>
</Tabs>

Choose one option. If you set both, the `release` option overrides `LEMMA_RELEASE`.
