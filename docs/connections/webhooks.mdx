...
meta:
  contentType: "How-to"
  goal: "Receive signed HTTP POST requests for issue lifecycle events"
  audience: "People integrating Lemma issues into another system"
---

Project webhooks send a signed HTTP POST request whenever an issue changes state. They are separate from both [Slack](/connections/slack) alerts and [Lemma MCP](/connections/mcp). Open the project’s **Settings → Webhooks** in the [Lemma dashboard](https://platform.uselemma.ai) to configure one.

Lemma doesn’t emit `incident.*` events. Customer alerting follows issues instead. If you find incident-named events in older samples or docs, they’re stale.
...
<Steps>
  ...
  <Step title="Verify signature and timestamp">
    ...
    The signature is a Hash-based Message Authentication Code (HMAC) with SHA-256 over the exact raw body.
