...
icon: "/images/brands/invert/mastra.svg"
meta:
  contentType: "How-to"
  goal: "Record Mastra agent and workflow runs with Lemma"
  audience: "Engineers on Mastra"
---
...
A complete docs-chat agent using this integration lives in [`examples/mastra`](https://github.com/uselemma/lemma/tree/main/examples/mastra). See [Runnable examples](/guides/examples).
...
The trace name is the Mastra agent or workflow id (`entityId`), falling back to its display name (`entityName`), then to the id parsed out of run span names like `agent run: 'support-agent'` / `workflow run: 'nightly-report'`. Set `agentName` on the exporter to override it.

Mastra may set root `input` to `{ messages: [...] }`. Lemma extracts the latest user message as the trace root input so each turn stays readable; generations still carry the full message list Mastra sent to the model.
