...
icon: "/images/brands/openai-agents.svg"
meta:
  contentType: "How-to"
  goal: "Record OpenAI Agents SDK traces with Lemma"
  audience: "Engineers on the OpenAI Agents SDK"
---
...
You do not need to wrap agent runs in `lemma.trace()`. Register the processor once, then run OpenAI Agents normally. A complete docs-chat agent using this integration lives in [`examples/openai-agents`](https://github.com/uselemma/lemma/tree/main/examples/openai-agents) (TypeScript) and [`examples/python/openai-agents`](https://github.com/uselemma/lemma/tree/main/examples/python/openai-agents) (Python). See [Runnable examples](/guides/examples).
...
Soft tool failures (`isError` / `is_error` / Mastra `error: true`) mark the tool span as `ERROR` without failing the root when a later generation succeeds.
