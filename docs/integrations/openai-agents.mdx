...
once, then run OpenAI Agents normally. A complete docs-chat agent using this
integration lives in [`examples/openai-agents`](https://github.com/uselemma/lemma/tree/main/examples/openai-agents)
(TypeScript) and [`examples/python/openai-agents`](https://github.com/uselemma/lemma/tree/main/examples/python/openai-agents)
...
Call `forceFlush()` / `force_flush()` (or `shutdown()`) to send any still-open
...
The integration always records prompts, tool inputs, tool outputs, model output,
and error messages — a trace cannot show what a run produced or why it failed
without them. Redact secrets and sensitive user data before they reach the
agent.
