...
Six terms describe everything in Lemma: trace, span, generation, tool call, thread, and project. They are the same words used in the dashboard and across these docs.
...
A **trace** is a single end-to-end execution of your agent, from the user’s input to the final response. It records the input, the output (or error), timing, and the tree of spans inside it. Each trace has a unique trace ID.
...
A **tool call** is a span that represents a single tool invocation: its name, arguments, and result. Tool execution happens in your code, so you must record it explicitly to make it visible. See [Tool calls](/tracing/instrumentation/tool-calls).
