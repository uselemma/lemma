...
<Steps>
  <Step title="Optional: install the agent skill">
    If you want a coding agent to add tracing for you, install the Lemma skill from the public repo with the Vercel Skills CLI:

    ```bash
    npx skills add uselemma/lemma --skill "skills/lemma-tracing"
    ```

    Then prompt your agent to use the Lemma AI skill from `github.com/uselemma/lemma` at `skills/lemma-tracing`.
  </Step>
